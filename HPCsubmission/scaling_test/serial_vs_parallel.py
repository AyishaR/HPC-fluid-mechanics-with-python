...
class SerialVsParallel:
    ...
    def parse(self):
        ...
        parser.add_argument('-plot_path', type=str, 
                            default="plots/Serial_vs_parallel",
    ...
    def run(self):
        if os.path.exists(self.time_log_path):
            ...
            plt.plot(df[SERIAL].tolist(), df[PARALLEL].tolist(), 
                     marker='o', linestyle='-')
            plt.axis("equal")
            plt.xlabel('Serial Execution Time (seconds)')
            plt.ylabel('Parallel Execution Time (seconds)')
            plt.title(f'Comparison of execution time with different grid sizes - {self.config_title}', size=10, wrap=True)

            df.apply(lambda x: plt.annotate(f"{int(x['Grid'])}x{int(x['Grid'])}", (x[SERIAL], x[PARALLEL]), textcoords="offset points", xytext=(0,10), ha='center'), axis=1)
