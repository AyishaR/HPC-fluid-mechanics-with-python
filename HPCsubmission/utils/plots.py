...
def plot_decay(arr, label_x, label_y, title=None, init_arc=None, path=None):
    ...
    plt.setp(axes, xlabel=label_x, 
             ylabel=label_y, 
             ylim=(np.min(arr)*0.999, np.max(arr)*1.001))
    if title:
...
def plot_combined(self, periodic_data,
    ...
    for i in range(periodic_data.shape[0]):
        plt.plot(periodic_data[i][1:-1])
    plt.xlabel(label_x)
    plt.ylabel(label_y)
    if title:
        plt.title(title)
    plt.show()
    plt.savefig(f"{path}/{title}/{label_x}-{label_y}.png")
